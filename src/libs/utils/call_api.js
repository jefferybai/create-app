...
function callApi(option) {
    ...
    if(option.url == 'https://opas.firstleap.cn/api/paper/add' || (data && data.authority)) {
    ...
    let timeout = new Promise((resolve, reject) => {
        setTimeout(reject, 15000, {status: 1014, error: '当前网络不稳定，请刷新浏览器'});
    });
    //restfull api 处理规范降级
    let requestServer = new Promise((resolve, reject) => {
        fetch(url, params).
        then(fetchDone.bind(null, option.acceptType)).
        then((result) => {
            let res = JSON.parse(result);
            if(res.status === 1001 || res.status === 2001) {
                resolve(res);
            } else {
                reject(res)
            }
    ...
    return Promise.race([timeout, requestServer]);
